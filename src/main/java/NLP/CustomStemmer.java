...
public class CustomStemmer {
	...
	private static final Set<Integer> vowelSet = new HashSet<>(
			Arrays.asList(new Integer[] { (int) 'a', (int) 'e', (int) 'i', (int) 'o', (int) 'u', (int) 'y' }));
	private static final Set<String> doubleSet = new HashSet<>(
			Arrays.asList(new String[] { "bb", "dd", "ff", "gg", "mm", "nn", "pp", "rr", "tt" }));

	private static final Set<String> specialSet = new HashSet<>(
			Arrays.asList(new String[] { "at", "bl", "iz", "dl", "gl", "pl", "tl", "kl", "tl", "dg", "iv", "tr", "dg",
					"uc", "rc", "ev", "rg", "fl", "ib", "av", "ng", "um", "ul", "lv", "nc", "rv", "rs", "ur" }));
	...
	public String stemComparativeADJ(String comparativeADJ) {
		...
		if (type == 0)
			seqWithoutPostfix = comparativeADJ.substring(0, comparativeADJ.length() - 2).toCharArray();
		if (type == 1)
			seqWithoutPostfix = comparativeADJ.substring(0, comparativeADJ.length() - 3).toCharArray();
	...
	public String stemNNS(String pluralNoun) {
		...
		char[] seq = pluralNoun.substring(0, pluralNoun.length() - 1).toCharArray();
		...
		case 'i':
			...
			return noun.toString();
		/*
		 * if (!vowelSet.contains(pluralNoun .charAt(length - 4))) {
		 * noun.append(pluralNoun.subSequence(0, length - 3)); noun.append('y'); } else
		 * noun.append(pluralNoun.subSequence(0, length - 1)); break;
		 */
		case 's':
			if (c3 == 's')
				noun.append(c2);
	...
	public String stemVBD(String verb) {
		...
		v.append(c1);
		try {
			double p0 = trainer.getProbability(c3, c2, c1, 'e') * trainer.getProbability(c2, c1, 'e', ' ');
			...
			if (pastSimpleVerb.charAt(length - 2) == 'e') {
				...
				if (vowelCount > 0) {
					if ((pastSimpleVerb.subSequence(0, length - 2).length() < 4 && vowelCount == 1)
							|| (pastSimpleVerb.charAt(length - 3) == 's' && pastSimpleVerb.charAt(length - 4) != 's')) {
						verb.append(pastSimpleVerb.subSequence(0, length - 2));
						verb.append("e");
					} else {
						if (doubleSet.contains(pastSimpleVerb.subSequence(length - 4, length - 2)))
							verb.append(pastSimpleVerb.subSequence(0, length - 3));
						else {
							if (specialSet.contains(pastSimpleVerb.subSequence(length - 4, length - 2))) {
								verb.append(pastSimpleVerb.subSequence(0, length - 2));
								verb.append("e");
							} else {
	...
	private boolean isRemovableDoubleConsonents(char c) {
		return c == 't' || c == 'n' || c == 'r' || c == 'd' || c == 'm' || c == 'f' || c == 'g' || c == 'p' || c == 'b';
	}

	private boolean isVowel(char c) {
		return c == 'e' || c == 'a' || c == 'o' || c == 'i' || c == 'u' || c == 'y';
	}

	public String verbPresentParticipleStem(final CharSequence presentParticipleVerb) {
		...
		if (presentParticipleVerb.charAt(length - 1) == 'g') {
			if (presentParticipleVerb.charAt(length - 2) == 'n') {
				if (presentParticipleVerb.charAt(length - 3) == 'i') {
					...
					if (vowelCount > 0) {
						if ((presentParticipleVerb.subSequence(0, length - 3).length() < 4 && vowelCount == 1
								&& vowelSet.contains(presentParticipleVerb.charAt(length - 5)))
								|| (presentParticipleVerb.charAt(length - 4) == 's'
										&& presentParticipleVerb.charAt(length - 5) != 's')) {
							verb.append(presentParticipleVerb.subSequence(0, length - 3));
							verb.append("e");
						} else {
							if (doubleSet.contains(presentParticipleVerb.subSequence(length - 5, length - 3)))
								verb.append(presentParticipleVerb.subSequence(0, length - 4));
							else {
								if (specialSet.contains(presentParticipleVerb.subSequence(length - 5, length - 3))) {
									verb.append(presentParticipleVerb.subSequence(0, length - 3));
									verb.append("e");
								} else {
									verb.append(presentParticipleVerb.subSequence(0, length - 3));
	...
	public String getStatisticString() {
		return "Mapped: " + STAT_mapperTimes + " - Stemmed: " + STAT_StemmingTimes + " - Passed: "
				+ STAT_KeepOriginalTimes;
	}
	...
	public String[] stem(String[] mayBePair, boolean lite) {
		// Do not stem if this word contains number, we have no apparent rules
		// for such words
		if (Util.hasNumeric(mayBePair[0]))
			return mayBePair;
		String mappedPair[] = IrregularMapper.getInstance().MapIrregular(mayBePair);
		if (mappedPair != null) {
			STAT_mapperTimes++;
			return mappedPair;
		}
		String result[] = null;
		if (mayBePair.length == 4)
			result = new String[] { null, null, mayBePair[2],mayBePair[3] };
		else
			result = new String[] { null, null };
		if (mayBePair[1].equals("JJR") || mayBePair[1].equals("JJS")) {
			result[0] = stemComparativeADJ(mayBePair[0]).intern();
			result[1] = "JJ".intern();
			result = SymSpell.getInstance().correctThisWord_POS(result);
			return result;
		}

		if (mayBePair[1].equals("NNS") || mayBePair[1].equals("NNPS")) {
			result[0] = stemNNS(mayBePair[0]).intern();
			result[1] = "NN".intern();
			result = SymSpell.getInstance().correctThisWord_POS(result);
			return result;
		} else {
			if (mayBePair[1].equals("VBD") || mayBePair[1].equals("VBG") || mayBePair[1].equals("VBN")
					|| mayBePair[1].equals("VBZ")) {
				switch (mayBePair[1]) {
				case "VBD":
					result[0] = stemVBD(mayBePair[0]).intern();
					break;
				case "VBG":
					if (lite)
						result[0] = mayBePair[0].intern();
					else
						result[0] = stemVBG(mayBePair[0]).intern();
					break;
				case "VBN":
					result[0] = stemVBN(mayBePair[0]).intern();
					break;
				case "VBZ":
					result[0] = stemVBZ(mayBePair[0]).intern();
					break;

				}
				if (mayBePair[1].equals("VBG") && lite)
					result[1] = mayBePair[1].intern();
				else {
					result[1] = "VB".intern();
					result = SymSpell.getInstance().correctThisWord_POS(result);
				}
				...
				if (result[1] == null || result[0] == null) {
					if (POSTagConverter.getInstance().getCode(mayBePair[1]) != -1)
						result = SymSpell.getInstance().correctThisWord_POS(mayBePair);
				} else {
