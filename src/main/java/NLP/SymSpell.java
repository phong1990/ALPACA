...
public class SymSpell implements Serializable {
	...
	public static final String TRAINDIRECTORY = TextNormalizer.getDictionaryDirectory() + "correctorTraining/";
	...
	public static SymSpell getInstance() {
		if (instance == null) {
			...
			instance.createDictionary(TextNormalizer.getDictionaryDirectory() + "baseWord/wordnet/",
					TextNormalizer.getDictionaryDirectory() + "baseWord/newwords/",
					TextNormalizer.getDictionaryDirectory() + "baseWord/misc/",
					TextNormalizer.getDictionaryDirectory() + "stop/", instance.baseDictionary);
	...
	private static Iterable<String> parseWords(String text) {
		...
		return Arrays.asList(Util.removeNonChars(text).toLowerCase().split(" "));
	}
	...
	private boolean createDictionaryEntry(String key, Map<String, DictionaryItem> dictionary) {
	...
	private void createFullDictionary(String corpus, String corpus2, String misc, String stop) {
		...
		System.out.println("DONE! full English dictionary now has " + fullDictionary.size() + " words");
	}
	...
	private void createDictionary(String corpus, String corpus2, String misc, String stop,
		...
		System.out.println(">>Dictionary created: " + wordCount + " words, " + dictionary.size()
				+ " entries, for edit distance=" + editDistanceMax);
	...
	private long readDictionaryFromDirectory(String misc, Map<String, DictionaryItem> dictionary, long wordCount) {
		Scanner br;
		br = null;
		try {
	...
	private static void addLowestDistance(List<EditItem> suggestions, EditItem suggestion) {
		...
		if ((verbose < 2) && (suggestions.size() > 0) && (suggestions.get(0).distance > suggestion.distance))
			suggestions.clear();
	...
	private List<EditItem> edits(String word, int editDistance, boolean recursion) {
		editDistance++;
			...
			for (int i = 0; i < word.length(); i++) {
				...
				if (!deletes.contains(delete)) {
					...
					if (recursion && (editDistance < editDistanceMax)) {
						...
						for (EditItem edit1 : edits(delete.term, editDistance, recursion)) {
	...
	private static int trueDistance(EditItem dictionaryOriginal, EditItem inputDelete, String inputOriginal) {
		...
		if (dictionaryOriginal.term == inputOriginal)
			...
			return computeDamerauLevenshteinDistance(dictionaryOriginal.term, inputOriginal);// adjust distance, if both
																								// distances>0
	}
	...
	class NameComparator implements Comparator {
		public int compare(Object arg0, Object arg1) {
			if (arg0 instanceof SuggestItem && arg1 instanceof SuggestItem) {
				...
				return ((SuggestItem) arg0).distance - ((SuggestItem) arg1).distance;
			}
			return 0;
		}
	}
	...
	private static void loadBlackList(HashSet<String> blackList, File file) throws FileNotFoundException {
	...
	private List<SuggestItem> lookup(String input, int editDistanceMax, Map<String, DictionaryItem> dictionary) {
		...
		while (candidates.size() > 0) {
			...
			if ((verbose < 2) && (suggestions.size() > 0) && (candidate.distance > suggestions.get(0).distance)) {
			...
			// add edits
			if (candidate.distance < editDistanceMax) {
				for (EditItem delete : edits(candidate.term, candidate.distance, false)) {
	...
	public String[] correctThisWord_POS(String[] input) {
		...
		String[] result = null;
		if (input.length == 4)
			result = new String[] { "", "", input[2], input[3] };
		else
			result = new String[] { "", "" };
		// display term and frequency
		if (suggestions.size() == 0) {
	...
	public String getStatisticString() {
		return "Corrected: " + STAT_SuccessfullyCorrected + " - Can't correct: " + STAT_NotCorrected;
	}
